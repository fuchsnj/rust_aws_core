...
impl SignedRequest{
	...
	pub fn get_headers(& self) -> &BTreeMap<String, Vec<Vec<u8>>> {
		&self.headers
	}
	...
	pub fn sign_and_execute(&mut self, creds: &Credentials) -> Response {
		...
		match self.get_payload() {
			...
			Some(payload) => {
				let payload_hash = &to_hexdigest_from_bytes(&payload);
				canonical_request = format!("{}\n{}\n{}\n{}\n{}\n{}",
					&self.method,
					self.canonical_uri,
					self.canonical_query_string,
				...
				self.remove_header("x-amz-content-sha256");
				self.add_header("x-amz-content-sha256", payload_hash);
				self.remove_header("content-length");
				self.add_header("content-length", &format!("{}", payload.len()));
			}
		}
		self.remove_header("content-type");
		self.add_header("content-type", "application/x-amz-json-1.0");
		...
		let signing_key = signing_key(&creds.get_secret(), date, &region_in_aws_format(self.region), &self.service);
		...
		let auth_header = format!("AWS4-HMAC-SHA256 Credential={}/{}, SignedHeaders={}, Signature={}",
	               &creds.get_id(), scope, signed_headers, signature);
